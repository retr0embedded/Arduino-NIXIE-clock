...
#define rtc_pseconds_reg  0x01
#define rtc_seconds_reg   0x02
#define rtc_minutes_reg   0x03
#define rtc_hours_reg     0x04
#define rtc_dow_reg     0x05
#define rtc_date_reg    0x06
#define rtc_month_reg     0x07  
#define rtc_year_reg    0x05
...
volatile bool displayseconds = false;
...
ISR(TIMER1_OVF_vect) {
    ...
    switch (digit) {
            case 1: if (hours >= 10) {
                    ...
                    if (DisplayingTemperature || displayseconds){
                     updateNixieTube(TUBE1_PIN, video_mem[1]);
                    } else {
                    digitalWrite(TUBE1_PIN, LOW); // Turn off the first tube if hours < 10 and not displaying the temperature or minutes/seconds
                    }
                } break;
...
if (!displayseconds){
  ...
  video_mem[1]= minutes / 10;
  video_mem[2]= minutes % 10;
  video_mem[3]= seconds/ 10;
  video_mem[4]= seconds % 10;  
